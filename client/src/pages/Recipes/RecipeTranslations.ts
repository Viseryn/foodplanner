import { Translations } from "@/hooks/useTranslation"

export const RecipeTranslations: Translations = {
    id: "recipe",

    en: {
        "dropdown.edit.recipe": "Edit recipe",
        "dropdown.delete.recipe": "Delete recipe",
        "dropdown.export.recipe": "Download recipe",
        "dropdown.refresh": "Refresh",

        "sab.label": "Plan meal",

        "ingredients.card.title.1": "Ingredients for",
        "ingredients.card.title.2": "portion",
        "ingredients.card.title.3": "portions",

        "ingredients.button.shoppinglist": "Add everything to shopping list",
        "ingredients.button.pantry": "Add everything to pantry",
        "ingredients.button.done": "Done!",

        "instructions.card.title": "Instructions",

        "externalUrl.card.title": "Linked recipe",
        "externalUrl.description": "The linked recipe will be opened in a new tab.",

        "empty.recipe.notification": "There's nothing to see here yet.",
        "empty.recipe.button.label": "Edit recipe",

        "topbar.title.add.recipe": "Add recipe",

        "label.title": "Recipe title",
        "label.sideDish": "Save as side dish",
        "label.portionSize": "How many portions?",
        "label.image": "Upload image",
        "label.externalUrl": "External link",
        "label.ingredients": "Ingredients",
        "label.instructions": "Instructions",

        "placeholder.ingredients": "250 ml vegetable broth\n1/2 tube tomato paste\n10 g salt",
        "placeholder.instructions": "Write each step into a separate line.",

        "button.save": "Save recipe",
    },

    de: {
        "dropdown.edit.recipe": "Rezept bearbeiten",
        "dropdown.delete.recipe": "Rezept löschen",
        "dropdown.export.recipe": "Rezept exportieren",
        "dropdown.refresh": "Aktualisieren",

        "sab.label": "Mahlzeit planen",

        "ingredients.card.title.1": "Zutaten für",
        "ingredients.card.title.2": "Portion",
        "ingredients.card.title.3": "Portionen",

        "ingredients.button.shoppinglist": "Alles auf die Einkaufsliste",
        "ingredients.button.pantry": "Alles in die Vorratskammer",
        "ingredients.button.done": "Erledigt!",

        "instructions.card.title": "Zubereitung",

        "externalUrl.card.title": "Verlinktes Rezept",
        "externalUrl.description": "Das verlinkte Rezept wird in einem neuen Tab geöffnet.",

        "empty.recipe.notification": "Hier gibt es noch nichts zu sehen.",
        "empty.recipe.button.label": "Rezept bearbeiten",

        "topbar.title.add.recipe": "Neues Rezept",

        "label.title": "Titel des Rezepts",
        "label.sideDish": "Als Beilage speichern",
        "label.portionSize": "Wie viele Portionen?",
        "label.image": "Bild hochladen",
        "label.externalUrl": "Externes Rezept verlinken",
        "label.ingredients": "Zutaten",
        "label.instructions": "Zubereitung",

        "placeholder.ingredients": "250 ml Gemüsebrühe\n1/2 Tube Tomatenmark\n10 g Salz",
        "placeholder.instructions": "Schreibe jeden Schritt in eine eigene Zeile.",

        "button.save": "Rezept speichern",
    },
}